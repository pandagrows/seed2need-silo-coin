...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s SILO too low! (Should be %s SILO)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 20) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s SILO too high! (Should be %s SILO)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "seed2need.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
        f.write("[regtest]\n")
...
def append_config(dirname, n, options):
    datadir = get_datadir_path(dirname, n)
    with open(os.path.join(datadir, "seed2need.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "seed2need.conf")):
        with open(os.path.join(datadir, "seed2need.conf"), 'r', encoding='utf8') as f:
            for line in f:
