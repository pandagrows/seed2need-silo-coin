...
class ReorgStakeTest(Seed2needTestFramework):
    ...
    def check_money_supply(self, expected_silo):
        # verify that nodes have the expected SILO supply
        silo_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(silo_supply, [DecimalAmt(expected_silo)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that SILO supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check SILO supply...")
