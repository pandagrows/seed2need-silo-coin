...
*Setup instructions for a gitian build of SEED2NEED Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SEED2NEED
...
to the SEED2NEED GitHub release page.
...
- [Building SEED2NEED Core](#building-seed2need-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/seed2need-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/seed2need-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/seed2need-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/seed2need-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/seed2need-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/seed2need-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/seed2need-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@seed2need.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/seed2need-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/seed2need-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/seed2need-osx-*-build.assert
```
