...
- [Development guidelines](#development-guidelines)
    - [General SEED2NEED Core](#general-seed2need-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SEED2NEED Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play SILOs (tSILO)" on the test network, if you
...
SEED2NEED Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/seed2need-project/seed2need/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_seed2need
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_seed2need --log_level=test_suite
$ valgrind -v --leak-check=full src/seed2needd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_seed2need.coverage/index.html`.
```

**Sanitizers**
SEED2NEED can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates SILOs (if wallet is enabled).
...
However, in open source software such as SEED2NEED Core, where everyone uses
...
pay attention to for reviewers of SEED2NEED Core code.

General SEED2NEED Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: SEED2NEED Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against SEED2NEED Core so that they can be integrated
...
- src/chiabls
  - Upstream at https://github.com/SEED2NEED-Project/bls-signatures/ ; actively maintained by Core contributors, deviates from upstream https://github.com/Chia-Network/bls-signatures/
...
file descriptors it will cause problems with SEED2NEED's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:pandagrows/seed2need-farm-coin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `seed2need-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `seed2need-cli` and the GUI debug console use this table to determine how to
