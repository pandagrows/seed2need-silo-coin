SEED2NEED Core version *v5.0.0* is now available from:  <https://github.com/seed2need-project/seed2need/releases>
...
SEED2NEED Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Sapling Parameters
In order to run, SEED2NEED Core now requires two files, `sapling-output.params` and `sapling-spend.params` (with total size ~50 MB), to be saved in a specific location.
...
For the other packages, the user must save the param files in the proper location, before being able to run SEED2NEED v5.0.0:
...
#### Overview
Users will be able to protect their financial information by sending and receiving SILOs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://www.seed2need.me .
...
* Top bar showing shield SILO balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield SILO balance.
...
See the release notes from [v4.2.0](https://github.com/pandagrows/seed2need-farm-coin/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed SILO supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in SILO
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in SILO
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zSILOsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield SILO)
  The `moneysupply` JSON return entry is the total SILO supply (the sum of all transparent and shield SILO)
...
| `getaccountaddress` | Returned the current SEED2NEED address associated with an internal account | Account system removal |
| `setaccount` | Associate a SEED2NEED address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given SEED2NEED address | Account system removal |
| `getaddressesbyaccount` | Returned a list of SEED2NEED addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a SEED2NEED address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a SEED2NEED address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zSILO balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zSILO mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zSILO mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zSILO amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zSILO to a SILO address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zSILO mints to a SILO address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zSILOs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzsiloseed` | Set the wallet's deterministic zSILO seed to a specific value | Zerocoin deprecation |
| `getzsiloseed` | Return the current deterministic zSILO seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zSILO seed | Zerocoin deprecation |
| `dzsilostate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzsilo` | Do a search for deterministically generated zSILO that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_seed2need unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake seed2needd and unit tests: fixing missing MacOS framework. (furszy)
