SEED2NEED Core version *3.1.1* is now available from:  <https://github.com/seed2need-project/seed2need/releases>
...
SEED2NEED Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zSILO created before block 1050020

The transition to v2 zSILO and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zSILO to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zSILO v1 will now be able to convert that into SILO and then zSILO v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zSILO stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by SEED2NEED stakers.
...
### Show progress percent for zsilo reindex operations
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zSILO code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zSILO elements if needed. (presstab)
 - #612 `6dd752cb5` [Qt] Show progress percent for zsilo reindex operations (Fuzzbawls)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zSILO created before block 1050020. (presstab)
