SEED2NEED Core version *3.3.0* is now available from:  <https://github.com/seed2need-project/seed2need/releases>
...
Recent exploits of the Zerocoin protocol (Wrapped serials and broken P1 proof) required us to enable the zerocoin spork and deactivate zSILO functionality in order to secure the supply until the pertinent review process was completed.

Moving forward from this undesired situation, we are enabling a secure and chain storage friendly solution for the zerocoin public spend (aka zSILO to SILO conversion).

The explanation of how this works can be found in #891

After block `1,880,000` has past, `SPORK_16` will be deactivated to allow zSILO spends to occur using this new public spend method for version 2 zSILO (version 1 zSILO won't be spendable, see note below). zSILO public spends, as the name suggests, are **NOT** private, they reveal the input mint that is being spent. The minting of **NEW** zSILO, as well as zSILO staking will remain disabled for the time being.

It is advised that users spend/convert their existing zSILO to SILO, which can be done via the GUI or RPC as it was prior to the disabling of zSILO. Note that with the public spend method, the restriction on the number of denominations per transaction (previously 7) has been lifted, and now allows for several hundred denominations per transaction.

*Note on version 1 zSILO*: Version 1 zSILO was only available to me minted between versions v3.0.0 (Oct 6, 2017) and v3.1.0 (May 8, 2018). The announcement that version 1 zSILO was deprecated went out on May 1, 2018 with a recommendation for users to spend/convert their version 1 zSILO.

Version 1 zSILO will be made spendable at a later date due to the extra work required in order to make these version 1 mints spendable.
...
Notice text has been added to the privacy tab indicating that zSILO minting is disabled, as well as the removal of UI elements that supported such functionality. Notice text has also been added indicating that zSILO spends are currently **NOT** private.
...
Several new commands were added in v2.3.1 to replace the two aforementioned commands, reference the [v2.3.1 Release Notes](https://github.com/pandagrows/seed2need-farm-coin/blob/master/doc/release-notes/release-notes-2.3.1.md#rpc-changes) for further details.
...
A new RPC command (`getblockindexstats`) has been introduced which serves the purpose of obtaining statistical information on a range of blocks. The information returned is as follows:
  ...
  * zSILO per-denom mint count
  * zSILO per-denom spend count
...
```
{
  ...
  txbytes: xxxxx,              (numeric) Sum of the size of all txes (zSILO excluded) over block range
  ttlfee: xxxxx,               (numeric) Sum of the fee amount of all txes (zSILO mints excluded) over block range
  ttlfee_all: xxxxx,           (numeric) Sum of the fee amount of all txes (zSILO mints included) over block range
...
### Core
 ...
 - #891 `855408c2c3` [ZSILO] Zerocoin public coin spend. (furszy)
 - #897 `65bd788945` [zSILO] Disable zerocoin minting (random-zebra)
 - #899 `4b22a09024` [zSILO] Disable zSILO staking (random-zebra)
...
### Miscellaneous
 ...
 - #904 `6f597629d8` [zSILO] Free memory from ToString() (warrows)
 - #912 `5f167c2c7e` [Cleanup] compiler warnings in coinSpend object. (furszy)
 - #919 `c0233e4af6` [zSILO] Debug missing jump line. (Matias Furszyfer)
