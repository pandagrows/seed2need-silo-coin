...
The build script we'll be using is contained in the SEED2NEED github repository ([contrib/gitian-build.py](https://github.com/seed2need-project/seed2need/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the SEED2NEED repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/pandagrows/seed2need-farm-coin/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, seed2need-detached-sigs, and seed2need GitHub repos.
