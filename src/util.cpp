...
const char * const SEED2NEED_CONF_FILENAME = "seed2need.conf";
const char * const SEED2NEED_PID_FILENAME = "seed2need.pid";
const char * const SEED2NEED_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\SEED2NEED
// Windows >= Vista: C:\Users\Username\AppData\Roaming\SEED2NEED
// Mac: ~/Library/Application Support/SEED2NEED
// Unix: ~/.seed2need
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SEED2NEED";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SEED2NEEDParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SEED2NEEDParams
    // Mac: ~/Library/Application Support/SEED2NEEDParams
    // Unix: ~/.seed2need-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SEED2NEEDParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "SEED2NEEDParams";
#else
    // Unix
    return pathRet / ".seed2need-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/seed2need/sapling-spend.params";
        sapling_output = "/usr/share/seed2need/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/seed2need/sapling-spend.params";
            sapling_output = "/usr/local/share/seed2need/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", SEED2NEED_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", SEED2NEED_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty seed2need.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override seed2need.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", SEED2NEED_PID_FILENAME));
