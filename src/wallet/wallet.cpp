                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s SILO", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
...
bool CWallet::CreateCoinstakeOuts(const CSiloStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
