...
{
    return QCoreApplication::translate("seed2need-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in seed2need.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in seed2need.qrc)
...
/** Class encapsulating SEED2NEED Core startup and shutdown.
...
/** Main SEED2NEED application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (SEED2NEEDGUI)
...
{
    window = new SEED2NEEDGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &SEED2NEEDGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SEED2NEEDGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(SEED2NEEDGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SEED2NEEDGUI::DEFAULT_WALLET);
        ...
        // SEED2NEED: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SEED2NEEDGUI::handlePaymentRequest);
        connect(window, &SEED2NEEDGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. SEED2NEED can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(seed2need_locale);
    Q_INIT_RESOURCE(seed2need);
    ...
    /// 6. Determine availability of data directory and parse seed2need.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("SEED2NEED Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("SEED2NEED Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // seed2need: links repeatedly have their payment requests routed to this process:
