...
#include "qt/seed2need/forms/ui_send.h"
#include "qt/seed2need/guitransactionsutils.h"
#include "qt/seed2need/loadingdialog.h"
#include "qt/seed2need/optionbutton.h"
#include "qt/seed2need/qtutils.h"
#include "qt/seed2need/sendchangeaddressdialog.h"
...
SendWidget::SendWidget(SEED2NEEDGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a SEED2NEED URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-silo");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSILOSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSILOSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent SILOs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield SILOs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent SILOs to shield."));
    }
}
...
void SendWidget::onSILOSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto siloAdd = Standard::DecodeDestination(address.toStdString(), isStaking, isExchange, isShielded);

        if (!Standard::IsValidDestination(siloAdd) || isStaking) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(siloAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(siloAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
