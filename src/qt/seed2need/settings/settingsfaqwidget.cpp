...
#include "qt/seed2need/settings/settingsfaqwidget.h"
#include "qt/seed2need/settings/forms/ui_settingsfaqwidget.h"
...
SettingsFaqWidget::SettingsFaqWidget(SEED2NEEDGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableSILO,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableSILO,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableSILO,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("SEED2NEED is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("SEED2NEED utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableSILOContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received SEED2NEED requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your SEED2NEED wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableSILO->setText(unspendableSILOContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of SEED2NEED with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("seed2need-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.seed2need.me'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::SILO)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in SEED2NEED Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the SEED2NEED network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendableSILO, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableSILO);});
    ...
    if (parent)
        connect(parent, &SEED2NEEDGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
