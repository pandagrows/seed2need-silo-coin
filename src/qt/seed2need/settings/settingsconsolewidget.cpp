...
#include "qt/seed2need/settings/forms/ui_settingsconsolewidget.h"
...
#include "qt/seed2need/settings/moc_settingsconsolewidget.cpp"
...
SettingsConsoleWidget::SettingsConsoleWidget(SEED2NEEDGUI* _window, QWidget *parent) :
...
void SettingsConsoleWidget::clear(bool clearHistory)
{
    ...
    messageInternal(CMD_REPLY, (tr("Welcome to the SEED2NEED RPC console.") + "<br>" +
...
static bool PotentiallyDangerousCommand(const QString& cmd)
{
    ...
    if (cmd.size() >= 13 && cmd.leftRef(11) == "dumpprivkey") {
        // valid SEED2NEED Transparent Address
        std::vector<std::string> args;
    ...
    if (cmd.size() >= 18 && cmd.leftRef(16) == "exportsaplingkey") {
        // valid SEED2NEED Shield Address
        std::vector<std::string> args;
        parseCommandLineSettings(args, cmd.toStdString());
