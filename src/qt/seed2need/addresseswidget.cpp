...
#include "qt/seed2need/forms/ui_addresseswidget.h"
#include "qt/seed2need/addresslabelrow.h"
#include "qt/seed2need/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(SEED2NEEDGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto siloAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(siloAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(siloAdd);
        if (walletModel->updateAddressBookLabels(siloAdd, label.toUtf8().constData(),
