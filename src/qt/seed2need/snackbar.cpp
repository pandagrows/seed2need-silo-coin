...
#include "qt/seed2need/forms/ui_snackbar.h"
#include "qt/seed2need/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SEED2NEEDGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SEED2NEEDGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
