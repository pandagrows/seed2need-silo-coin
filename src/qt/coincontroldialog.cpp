...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select SILO Outputs to Spend" :
            "Select Shielded SILO to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::SILO;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show SEED2NEED address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
