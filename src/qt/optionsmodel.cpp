...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SILO);
    ...
    if (settings.contains("nAnonymizeSeed2needAmount") || reset)
        gArgs.SoftSetArg("-anonymizeseed2needamount", settings.value("nAnonymizeSeed2needAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in seed2need.cpp during shotdown to also remove the window positions
