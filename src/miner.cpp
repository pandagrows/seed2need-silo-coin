...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("SEED2NEEDMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("SEED2NEEDMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SEED2NEEDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("seed2need-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running SEED2NEEDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("SEED2NEEDMiner exception");
    } catch (...) {
        LogPrintf("SEED2NEEDMiner exception");
    }

    LogPrintf("SEED2NEEDMiner exiting\n");
}
