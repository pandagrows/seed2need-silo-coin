...
bool CLegacyZSiloStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZSiloStake from non zc-spend", __func__);
    ...
    *this = CLegacyZSiloStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zsilo stake origin", __func__);

    // All good
    return true;
}

CLegacyZSiloStake::CLegacyZSiloStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZSiloStake::GetIndexFrom() const
{
...
CAmount CLegacyZSiloStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZSiloStake::GetUniqueness() const
{
...
bool CLegacyZSiloStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSILO stake block: height %d outside range", __func__, nHeight);
