...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SEED2NEEDParams"
    else
        PARAMS_DIR="$HOME/.seed2need-params"
    fi
fi
...
Failed to fetch the SEED2NEED zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
SEED2NEED - fetch-params.sh

This script will fetch the SEED2NEED zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SEED2NEED zkSNARK parameters. Note that it is
